...
chunk_pair_geo <- function(ATACpcs,
  ...
  if(nrow(ATACpcs)!=nrow(RNApcs)){
    ...
    for(i in 1:length(chunkList)){
      ...
      pairList[[i]] <-  cell_pairing(ATACpcs = chunkList[[i]][[1]],
                                     RNApcs = chunkList[[i]][[2]],
                                     mode = "geodesic",...)
    }
...
cell_pairing <- function(ATACpcs, # Input ATAC single cell PCs obtained from Running CCA dimensionality reduction. Shape: (n_ATAC_cells, n_PCs)
  ...
  if (mode == "geodesic"){
    ...
    for (subgraph_ind in unique(subgraphs$membership)){
      ...
      # THIS WAS ADDED AFTER WE REALIZED MAJOR SUBGRAPH(S) WERE BEING SKIPPED SOMETIMES
  ...
  # This is only if we want to return a list with extra info 
  #subgraph_clust <- subgraphs$membership
  #names(subgraph_clust) <- rownames(all_pcs)
  
  # return(list(pairs=all_pairs,
  #             subgraph_clust=subgraph_clust,
  #             UMAP_knn_k=umap_knn_k,
  #             mode=mode))
